...
| 分析与工程 | 波形图 / I/O Graph / 仪表盘、协议帧模式（粘包组帧）、多会话标签、`.ctproj` 工程与示例、桥接与文件传输、三语与在线更新 |
...
- 右下：版本号 `v1.8.3`（从 `version.py` 同步），有新版时变成「● 可更新 vX」可点徽标；左侧显示当前实时记录文件路径（📝）
...
├── latest.json             在线更新版本清单（version / url / sha256 / size / notes）
│
...
│   ├── version.py          版本号单点真源 (__version__ = "1.8.3")
...
- **按会话独立的引擎**：序列、脚本、Modbus 主机、录制、宏、发送 DSL、文件传输、回放、设备扫描、自动应答每个标签可各自运行；脚本控制台日志按标签隔离；Modbus 从机 bank 每会话一份；**可切标签**；关闭忙标签前需先停任务
- **多条循环发送**按会话独立：切标签后后台会话循环继续（与定时发送一致）；分组列表仍整窗共享
- 后台标签 RX 更新显示/统计，并喂入本会话引擎、自动应答与触发器（绘图 / 组帧 / 仪表盘仍只跟活动标签）
- 序列对话框：结果列跟当前标签；汇总行可看到其它标签正在跑的序列
...
- **v24 (v1.1.3)**: bug 修复版（功能同 1.1.2）。①修数据区拖选后新数据到达把选区延伸覆盖刚收到的行（原生文本选区随末尾插入延伸 → append 时按原偏移钉住选区）；②修无边框标题栏拖到屏幕顶部外消失、再也拖不回来（拖动加屏幕工作区边界钳制，标题栏始终留在屏内可抓，多显示器适配）；③修「检查更新」对话框更新说明过长时被截断、图标顶部被裁（更新说明放进限高滚动区、对话框按内容自适应高度）。
- **v25 (v1.1.4)**: **自动应答「帧头+长度组帧」+ 多条发送增强 + 窗口按钮**。自动应答新增第三种组帧：填帧头(如 `AA BB`)+长度字段偏移/宽度/字节序/整帧固定开销，维护跨包字节流、按帧头定位 + Length 算真实帧边界切分，正确处理串口粘包/拆包（优先于"整包静默超时"；`binproto.iter_length_frames`，自动应答对话框顶部配置、随会话配置档导入导出）。多条发送：名称列↔数据列间加可拖 `QSplitter`（拖一行所有行同步、长名称可拖宽看全）+ 顶部「全选 / 全不选」三态框（反映当前勾选）。多条发送、关键字高亮对话框补最小化 / 最大化按钮。
- **v26 (v1.1.5)**: **自动应答五项增强（校验段 / 规则测试器 / 命中统计 / 故障注入 / 范围延时）**。①**校验段**：每条应答可加 N 段内层/额外校验（对 `[起始..结束]` 算校验、覆盖到指定偏移或追加），在行尾 cs 之前按序计算，配合 `{rN}` 占位符回显可自动重算内层 CRC（解决「外层 Sum + 内层 CRC」只能硬编码）。②**规则测试器**（顶栏「测试」）：离线输入一帧 → 看命中哪条规则 + 应答预览（与实发共用组装逻辑、保证预览=实发；不发送 / 不计数 / 不推进 `{seq}`）。③**命中统计**：每行实时「命中 N」+「重置统计」（匹配且收包校验通过即计，运行态、不持久化）。④**故障注入**（全局框）：按概率 丢包 / 错CRC(末字节翻转) / 错长度(砍末字节)，专测主机重传与容错，被注入帧数据区留痕。⑤**范围延时**：延时字段支持 `100-300`（每次发随机抖动，模拟 turnaround）。两框右侧加一句话说明 + `?` 例子帮助窗；三语 i18n；无新依赖；经两轮多代理对抗式评审修复。
- **v27 (v1.1.6)**: **自动应答·多步状态机**。弹窗新增「状态机」框（启用 / 初始状态 / 当前状态实时显示 / 重置）；每条规则可选 **仅状态(when)**（当前状态等于它才命中，可逗号分隔多状态、状态名可含空格、空=通配）+ **跳转(goto)**（应答发出后切到该状态），把多条规则串成按帧序列推进的握手 / 会话（每行 when/goto 两列仅状态机启用时显示）。离线测试器同步显示当前状态 / 命中后跳转 / 「内容命中但状态不符」提示。工程化细节：goto 仅在应答**真正发出**后才推进（延时未到 / 断连 / 无客户端 / 发送失败 / 丢包不推进，主机重试仍命中当前状态）；带 goto 的应答整条（多段 + 延时）发完前后续帧进有界 FIFO 按收帧顺序处理（杜绝随机延时下状态乱序）；断连/重连/切总开关/导入/手动重置回到初始、编辑规则不打断握手。状态机默认关闭，关时与 v1.1.5 行为一致；三语 i18n；无新依赖；经四轮（云端 + 本地多代理）对抗式评审收敛。
- **v28 (v1.1.7)**: **自动应答·Modbus RTU 从机**。自动应答弹窗顶栏新增「Modbus 从机」按钮：启用 + 从机地址 + 寄存器表（按「空间 · 起始地址 · 值」配初值，地址支持 `0x`/十进制、线圈 0/1）。开启后整条引擎作为 Modbus 从机自动应答主机读写——读 `01` 线圈/`02` 离散输入/`03` 保持寄存器/`04` 输入寄存器，写 `05` 单线圈/`06` 单寄存器/`0F` 多线圈/`10` 多寄存器（写改运行态寄存器、可读回），非法功能/越界地址/非法数据回**异常响应**（`0x80|func`+异常码）。组帧：RTU 无帧头 → 按「功能码长度 + CRC 自洽」双重判定切帧 + 跨包缓冲，CRC 不符逐字节重同步（杜绝噪声/错位后永久失步），未知功能码 CRC 探测、`0F/10` 假写多头不阻塞后续合法帧。作 TCP 服务器多客户端时响应精确回请求方（每客户端独立半包缓冲）；C6 故障注入仍作用于 Modbus 响应。开启 Modbus 时规则/状态机/帧头组帧让位、弹窗收起这些区块只留故障注入 + 醒目提示条（消除「看着启用却不响应」）。另修暗色主题下复选框看不出选中/未选中（`::indicator` 样式）。新增 `tests/test_modbus_slave.py`（仓库首个自动化测试）；三语 i18n；无新依赖；Modbus 默认关、关时与 v1.1.6 行为一致；经云端 + 本地多代理对抗式评审收敛。
- **v29 (v1.1.8)**: **自动应答·位掩码 / 字段级匹配**。「匹配」从「逐字节精确 / 整字节 `??` 通配」升级为**逐字节 (值, 掩码) 比对**（命中需 `(收到字节 & 掩码) == (值 & 掩码)`），支持子字节粒度：**半字节通配** `A?` / `?5`（高 / 低 4 位，`X` 同 `?`）、**位级掩码** `b:` + 8 位 `0/1/x`（`x`=该位不关心，如 `b:1xxxxxx1` 只看最高与最低位）；**字段级**用通配填充到偏移 + 「前缀」模式表达。空格仅作分隔、纯 HEX 跨空格拼接（`A B`=0xAB，**向后兼容**）；三种匹配模式（包含 / 相等 / 前缀）对掩码同样生效。整字节精确 `AB`、整字节通配 `??` / `XX` 解析结果与改前**逐字节等价，旧规则零行为变化**；仅 HEX 模式有意义。改动集中在两个静态方法（`_ar_parse_hex_pat` / `_ar_hex_at`，仅被实时匹配与离线规则测试器共用），二者自动获得新能力；三语 i18n（占位符 + 帮助补「位 / 半字节掩码」一节）；新增 `tests/test_match_mask.py`（17 项，含向后兼容 / 坏格式 / 三模式）；无新依赖；经对抗式评审收敛。
- **v30 (v1.1.9)**: **自动应答·脚本应答**。每条规则可定义 Python `reply(frame, ctx)` 动态返回单帧/多帧；ctx 提供状态、序号、命中数和 CRC/校验工具。脚本在独立进程执行，超时整组终止；预览与实发进程隔离；导入含脚本配置前确认信任。新增进程隔离、超时、CRC、配置门禁自动化测试。
- **v31 (v1.2.0)**: **Modbus 主机轮询（master / poll）**——继自动应答的 Modbus RTU **从机**之后，补上 **主机**侧。数据区标题栏新增「Modbus 主机」按钮，弹窗每行配 `名称 / 从机ID / 功能码 / 起始地址 / 数量(或写值) / 周期ms`，按各自周期**半双工轮询**从机（一次只在途一条请求，收响应或超时 1s 再发下一条），实时显示**值（十进制 + 十六进制）/ 状态（OK / 超时 / 异常码 / 响应无效 / 发送失败）**。读 01 线圈 / 02 离散输入 / 03 保持 / 04 输入寄存器，写 05 单线圈 / 06 单寄存器（周期到点重复写、显示回显）。**传输**：自动（串口→Modbus RTU、TCP Client→Modbus TCP）或强制选择。**本地回显**模式：串口适配器回显发出帧时勾选——剥掉前导回显（容忍噪声/残留）再解析真实响应，专治 05/06 回显与「写成功」同形导致异常被吞。响应严格核对从机ID / 数量 / 写回显 / 异常功能码匹配；RTU 坏帧丢 1 字节重同步（不清整缓冲，杜绝本地回显粘包丢响应），TCP 按 MBAP 自分帧。新增纯逻辑 `src/modbus_master.py` + 对话框 `src/modbus_master_dialog.py` + `tests/test_modbus_master.py`（34 项）；三语 i18n；无新依赖；启用前不影响原有行为；经**三轮对抗式评审**收敛。
- **v32 (v1.2.1)**: **Modbus 主机增强 + 误操作安全 + 表格交互**。①写功能码补 **0F 写多线圈 / 10 写多寄存器**（「数量/写值」格填逗号或空格分隔的多值，如 `100,200,300`；0F 线圈填 0/1）。②**误操作安全**（经多轮对抗式评审收敛）：非法/越界 **从机ID(0..247)/起始地址(0..65535)/写值** 一律**拒发**而非静默钳位（杜绝非法 ID 变广播地址 0 误写所有从机）；写多任一项非法或超规范上限**整组拒发**（不静默截断/前移地址）；写功能码异常响应严格核对原功能码；**响应超时静默期**——超时后短暂不发下一条，避免迟到响应被误配对到下一行。③**草稿编辑**：启用轮询时编辑规则只进草稿、不再 300ms 自动提交并发送，须点**「应用」**才生效（防读切写时误发）；未应用草稿关窗重开保留。④**表格**：名称~值七列放进可拖 `QSplitter`，**列边界可左右拖宽、所有行+表头像素同步**，功能码列变窄。⑤健壮性：单条规则损坏只跳过不清空整表；字符串 `"false"` 正确识别为停用；大量非法规则异步排程不再递归致栈溢出；主机轮询激活时跳过自动应答，防与内置 Modbus 从机互发干扰；自动应答与 Modbus 主机**一键互斥**（开一个自动关另一个、UI 同步）、TCP/串口半帧写入即时断流重连不污染后续帧、Modbus 数值输入兼容前导零（如 `08`/`010`）。`tests/test_modbus_master.py` 增至 60+ 项；无新依赖；经**六轮对抗式评审**收敛。
...
- **v70 (v1.8.2)**: **正式版** — **弹窗全面主题化**（删除预设确认、帧构造器/Modbus 主机/脚本控制台命名输入、8 窗口上限提示全部换主题化对话框，跟随语言与主题）；**发送框粘贴只取纯文本**（不再带入网页/Word 的富文本格式）。Windows Setup / onefile 发布，macOS / Linux 新版待补包。1807 collected 全量通过。
- **v71 (v1.8.3)**: **正式版** — **启动提速**（启动按已存主题只应用一次样式、全局事件过滤器对无关事件直接放行、RTT 器件表枚举推迟到主窗口首帧之后）；安装版启动到首帧约 1.35 s → 1.03 s（上次停在 RTT 约 1.69 s → 1.03 s），界面外观不变。Windows Setup / onefile 与 Linux x86_64 已发，macOS 新版待补包。1815 collected 全量通过。
