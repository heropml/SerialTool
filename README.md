...
- **状态栏**
  ...
  - 右下：版本号 `v1.3.7`（从 `version.py` 同步），有新版时变成「● 可更新 vX」可点徽标；左侧显示当前实时记录文件路径（📝）
...
│   └── version.py          版本号单点真源 (__version__ = "1.3.7")
│
├── docs/                   文档
...
py -3 -m PyInstaller --noconfirm --clean --windowed --onefile ^
    --name CommTool_onefile_v1.3.7 --icon assets\icon.ico ^
