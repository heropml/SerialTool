...
- **v71 (v1.8.3)**: **正式版** — **启动提速**（启动按已存主题只应用一次样式、全局事件过滤器对无关事件直接放行、RTT 器件表枚举推迟到主窗口首帧之后）；安装版启动到首帧约 1.35 s → 1.03 s（上次停在 RTT 约 1.69 s → 1.03 s），界面外观不变。Windows Setup / onefile 发布，macOS / Linux 新版待补包。1815 collected 全量通过。
