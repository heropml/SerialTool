...
iOS 风格的串口调试工具，基于 PyQt5 + pyserial。支持中英文动态切换、无边框窗口 + 自绘标题栏、QSplitter 可拖拽布局、Xshell 风格日志、10 种校验算法、关闭确认 + 系统托盘、在线更新（关于 → 检查更新）、图标固化在源码内（防替换）。
...
**关于 → 检查更新**（v1.0.5）：系统托盘右键菜单新增「关于」入口，打开对话框显示图标 / 名称 / 版本 / 简介 + 「检查更新」按钮。

- 点「检查更新」自动从更新源读取版本清单（`latest.json`），与当前版本比对
- **有新版** → 显示版本号 + 更新说明 + 「下载并更新」→ 下载（带进度）→ 下载完弹出**正常安装向导**，手动点「下一步 / 安装」完成升级（非静默）
- **更新源顺序**：内网优先、自动回退 GitHub raw；每个源 **8 秒超时**，外网也不会卡在「正在检查…」
- 下载后校验文件头魔数（`MZ`）防错误页误执行；关闭对话框 / ESC 自动中止在途下载并删半成品；启动时清理 `%TEMP%` 残留安装包
- 基于 Qt 自带 **QtNetwork**，无新依赖；更新源地址在 `src/updater.py` 的 `UPDATE_MANIFEST_URLS` 配置，版本清单格式见根目录 `latest.json`
...
│
├── src/                    Python 源码（按模块拆分）
│   ├── main.py             入口：HiDPI + QApplication + 启动 SerialTool
│   ├── main_window.py      主窗口 SerialTool 主体类（最大模块）
...
├── dist_onefile/           PyInstaller --onefile 输出 SerialTool_onefile_v*.exe
└── installer/              Inno Setup 输出 SerialTool_Setup_v*.exe
```

> 运行时用户配置 `settings.ini`：打包版落在 exe 同级（装不进 Program Files 时回退 `%APPDATA%\SerialTool\`）；开发模式落在源码同级（已 gitignore）。
...
自动从 `src\version.py` 读版本号，输出 `dist_onefile\SerialTool_onefile_v<版本>.exe`（~37 MB 单文件，文件名带版本号，与安装包一致）。首次启动稍慢 1~2 秒（自解压到 `%TEMP%`），之后跟文件夹版无差。

等价命令（`v%VER%` 由脚本从 version.py 注入到 `--name`）：

```powershell
py -3 -m PyInstaller --noconfirm --clean --windowed --onefile ^
    --name SerialTool_onefile_v1.0.5 --icon assets\icon.ico ^
...
自动从 `src\version.py` 读 `__version__`，传给 ISCC 编译 `scripts\SerialTool.iss`，输出 `installer\SerialTool_Setup_v<版本>.exe` (~27 MB)。
...
- **v17 (v1.0.4)**: 边界鲁棒性修复 —— 定时发送时数据格式错误等任何发送失败都自动停（不再每周期 toast 轰炸）；`close_serial` 关闭时同步停掉多条发送循环（避免串口出错/断开后再弹一次「串口未打开」的双重提示）；`_settings_file` 写测试文件成功即判目录可写、删测试文件改 best-effort（`os.remove` 失败不再误判为不可写、不残留空文件）
- **v18 (v1.0.5)**: 在线更新 —— 托盘「关于」对话框新增「检查更新」，从版本清单（内网优先、回退 GitHub raw）比对最新版，一键下载并弹出安装向导手动升级；更新源 8 秒超时回退（外网不卡）、下载魔数（MZ）校验防错误页误执行、关闭对话框中止在途下载、启动清理 `%TEMP%` 残留安装包、版本号解析容错；基于 Qt 自带 QtNetwork 无新依赖；新增一键发版脚本 `scripts/release.ps1`
- **v19 (v1.0.6)**: 数据区搜索（Ctrl+F：高亮全部匹配 + ▲/▼ 上下跳转 + 实时「N/总数」计数）+ 多屏下状态栏/窗口拖拽/任务栏最小化修复（WM_GETMINMAXINFO 多显示器对齐）+ 深色主题搜索/高亮对比优化 + 系统托盘单击 toggle 显示/隐藏
- **v20 (v1.0.7)**: 稳定性与代码审查修复 —— 关闭串口时先断开读取线程(SerialReader)信号再停止（消除出错关闭路径的重复错误提示与重入）、读取线程退出等待余量加大（设备异常时不悬挂）；搜索导航改 O(1) 着色（大文档点 ▲/▼ 不再全文重扫）、打开搜索消除双重扫描、查找栏 viewport/计数标签加守卫、_parse_version 修正预发布版本号比较；修复纯搜索（未配关键字规则）时实时新数据的搜索匹配/计数停更
- **v21 (v1.0.8)**: 修复关键字 / 搜索高亮时，底部新接收的数据会「整批闪一下高亮」的问题（高亮选区不再随末尾插入延伸）
