...
- **只显高亮行**（v1.0.1）— 数据区选项里的开关，只保留命中关键字的行（其余折叠隐藏，数据不丢）
...
- **多条发送**（v1.0.1）— 「多条发送」按钮打开弹窗：每行一条数据 + 复选框，**每行可独立设 HEX / 换行 / 校验**；勾选多条后「循环发送」按设定间隔依次轮发；条目持久化
...
├── version.py              版本号单点真源 (__version__ = "1.0.1")
