...
TR = {
    "zh": {
        ...
        "mbm_add": "添加",
        "mbm_help_btn": "使用说明",
        "mbm_hint": "按每行周期轮询从机：串口走 Modbus RTU、TCP Client 走 Modbus TCP。读类显示寄存器/线圈值，写类(05/06)用「数量/写值」格当写入值。需先连接、并勾选上方「启用轮询」。",
        "mbm_col_name": "名称",
        "mbm_col_unit": "从机ID",
        "mbm_col_func": "功能码",
        "mbm_col_addr": "起始地址",
    ...
    "en": {
        ...
        "mbm_add": "Add",
        "mbm_help_btn": "Help",
        "mbm_hint": "Polls slaves at each row's period: serial uses Modbus RTU, TCP Client uses Modbus TCP. Read functions show register/coil values; write functions (05/06) use the Qty/Value cell as the value. Connect first and tick 'Enable polling' above.",
        "mbm_col_name": "Name",
        "mbm_col_unit": "Unit",
    ...
    "zh_tw": {
        ...
        "mbm_add": "新增",
        "mbm_help_btn": "使用說明",
        "mbm_hint": "依每行週期輪詢從機：序列埠走 Modbus RTU、TCP Client 走 Modbus TCP。讀類顯示暫存器/線圈值，寫類(05/06)用「數量/寫值」格當寫入值。需先連線並勾選上方「啟用輪詢」。",
        "mbm_col_name": "名稱",
        "mbm_col_unit": "從機ID",
        "mbm_col_func": "功能碼",
        "mbm_col_addr": "起始位址",
