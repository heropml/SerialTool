...
TR = {
    "zh": {
        ...
        "ble_scan_use": "使用此设备",
        "ble_scan_hint": "双击一行，或选中后点「使用此设备」。结果按地址去重。「过滤」打开后按勾选规则筛设备（默认同隐藏无名空包）。可选中一行看底部广播明细（类型、Flags、外观名、厂商名、服务个数、上次连接、服务数据、RAW）；悬停仍可看摘要。可按名称 / 地址 / UUID / 厂商搜索。打开「RSSI 过滤」后用滑条隐藏弱信号。一段时间无广播的行会变灰。",
        "ble_scan_search_ph": "搜索名称、地址、UUID 或厂商…",
        "ble_scan_filter": "过滤",
        "ble_scan_filter_tip": "打开后按下方勾选的规则过滤；关掉则显示全部。",
    ...
    "zh_tw": {
        ...
        "ble_scan_use": "使用此裝置",
        "ble_scan_hint": "雙擊一列，或選取後點「使用此裝置」。結果按位址去重。「過濾」打開後按勾選規則篩裝置（預設同隱藏無名空包）。可選取一列看底部廣播明細（類型、Flags、外觀名、廠商名、服務個數、上次連線、服務資料、RAW）；懸停仍可看摘要。可按名稱 / 位址 / UUID / 廠商搜尋。打開「RSSI 過濾」後用滑條隱藏弱訊號。一段時間無廣播的列會變灰。",
        "ble_scan_search_ph": "搜尋名稱、位址、UUID 或廠商…",
        "ble_scan_filter": "過濾",
        "ble_scan_filter_tip": "打開後按下方勾選的規則過濾；關掉則顯示全部。",
