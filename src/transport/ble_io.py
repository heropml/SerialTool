...
# Scan / connect / GATT write: library + OS errors. Shutdown/cancel/join keep
# a broader catch so a dying adapter cannot take down the BLE thread.
_BLE_OP_ERRORS = (OSError, RuntimeError, TypeError, ValueError, asyncio.TimeoutError)
try:
    from bleak.exc import BleakError
    _BLE_OP_ERRORS = (BleakError,) + _BLE_OP_ERRORS
except ImportError:
    pass
...
class BleScanner(QObject):
    ...
    def start(self):
        ...
        except _BLE_OP_ERRORS as exc:
            self._scanning = False
            _log.debug("BLE scan submit failed: %s", exc, exc_info=True)
...
class BleConn(QObject):
    ...
    def open(self):
        ...
        try:
            ...
            _log.debug("BLE open submit failed: %s", exc, exc_info=True)
