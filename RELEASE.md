...
- Windows 发版脚本：[`scripts/release.ps1`](scripts/release.ps1)（PowerShell，目前只推 GitHub，Gitee 步骤手动，见第六节）
...
```
自动完成：改版本号 → 改 `latest.json` → 打包 `.exe` → git 提交 → push GitHub →
建 GitHub Release `comm-v1.1.2` 上传 `.exe`。
**然后手动补 Gitee 那一份**（见第六节），否则国内用户收不到。
