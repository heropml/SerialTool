...
> 基线：**v1.5.7**（2026-08-14）  
> 综合：阶段 A/B 收口事实、v1.5.6 多会话补齐、v1.5.7 PCAP/体验打磨。  
...
| 版本策略 | 阶段 A/B → **v1.5.3 已发**；打磨向 → **v1.5.4–v1.5.7**；**v1.6** 协议流组帧 / 解析诊断（进行中） |
...
| 指标 | 数值 | 备注 |
|---|---|---|
| 发布版 | `comm-v1.5.7` | Win Setup/onefile + Mac DMG + Linux `.run`；`url_mac` / `url_linux` 已写入 |
| 测试 | **1574 collected / 11 skipped** | Windows 按文件隔离 pytest + macOS smoke |
...
5. **v1.6** 协议流组帧与解析诊断（进行中）：分析层按完整协议帧解析，默认收包模式兼容旧工程
...
- **v1.5.7 之后「接下来做什么」** → 以本文件 §7 为准（当前主线 v1.6 组帧）。  
