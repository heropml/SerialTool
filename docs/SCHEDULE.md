...
| # | 项 | 量 | 完成标准 |
|---|---|---|---|
| **B1** | macOS CI 烟雾 | 0.5–1d | `macos-latest` 跑 pytest 子集，防启动期 Qt 回归 |
| **B2** | ruff + pre-commit | 0.5d | 零门槛起步，只拦明显错误 |
| **B3** | pytest-cov 收集 | 0.3d | CI 上传/展示覆盖率；**先不设硬门槛** |

### 4.2 产品打磨（三选一为主线）

| # | 选项 | 量 | 何时选 |
|---|---|---|---|
| **B4a** | 多条循环发送 per-session | 1–2d | 与定时发送语义对齐（**默认推荐**） |
| **B4b** | 网关 UI 暴露 timeout / unit_map | 0.5–1d | 产线网关要可配映射/超时 |
| **B4c** | README 补多会话边界 3–5 条 | 0.2d | 成本最低的体验补强 |
...
4. **B1** macOS CI 烟雾 ← 下一刀  
5. **B4** 多会话择一（默认 B4a 或 B4c）+ **B2/B3** 质量门禁  
