...
> 基线：**v1.6.0**（2026-08-15）  
> 综合：阶段 A/B 收口事实、v1.5.6 多会话补齐、v1.5.7 PCAP/体验打磨、v1.6.0 协议流组帧。  
...
| 版本策略 | 阶段 A/B → **v1.5.3 已发**；打磨向 → **v1.5.4–v1.5.7**；**v1.6.0 已发**（协议流组帧 / 解析诊断） |
...
| 指标 | 数值 | 备注 |
|---|---|---|
| 发布版 | `comm-v1.6.0` | 本轮 Windows Setup/onefile；Mac DMG / Linux `.run` 补同一 tag 后写入 `url_mac` / `url_linux` |
| 测试 | **1594 passed / 11 skipped** | Windows 按文件隔离 pytest + macOS smoke |
...
- **v1.6.0 之后「接下来做什么」** → 以本文件 §5 / §7 为准（阶段 C 按触发条件；不上 P2）。  
