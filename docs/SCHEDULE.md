...
> 基线：**v1.7.4**（2026-08-24）  
> 综合：阶段 A/B 收口事实、v1.5.6 多会话补齐、v1.5.7 PCAP/体验打磨、v1.6.0 协议流组帧、v1.7.0 Windows BLE、v1.7.1 扫描打磨、v1.7.2 领域分包与 B6-3、v1.7.3 配置目录 / 事件总线 / B6-4、v1.7.4 更新完整性 / Webhook 边界 / 诊断与快速开始。  
...
| 版本策略 | 阶段 A/B → **v1.5.3 已发**；打磨向 → **v1.5.4–v1.5.7**；**v1.6.0 已发**（协议流组帧）；**v1.7.0 已发**（Windows BLE 主机 UART）；**v1.7.1 已发**（扫描打磨）；**v1.7.2 已发**（领域分包 / B6-3）；**v1.7.3 已发**（配置目录 / 事件总线 / B6-4）；**v1.7.4**（更新完整性 / Webhook 边界 / 诊断与快速开始） |
...
| 指标 | 数值 | 备注 |
|---|---|---|
| 发布版 | `comm-v1.7.4` | Windows Setup/onefile + macOS DMG + Linux x86_64 `.run`；清单须带各平台 `sha256` / `size` |
...
10. **v1.7.4**（更新清单 SHA-256/大小、Webhook 公网 HTTPS 钉 IP、诊断包与快速开始）
11. **不上**覆盖率硬门槛（B3 只收集，见 §6）
12. 阶段 C 剩余项（Mac 公证）与 P2 / PyQt6 仅按触发条件启动；Linux CI 烟雾已加
...
- **v1.7.4 之后「接下来做什么」** → 以本文件 §5 / §7 为准（阶段 C 按触发条件；不上 P2）。  
