...
> 基线：**v1.8.0**（2026-09-08）
> 综合：阶段 A/B 收口事实、v1.5.6 多会话补齐、v1.5.7 PCAP/体验打磨、v1.6.0 协议流组帧、v1.7.0 Windows BLE、v1.7.1 扫描打磨、v1.7.2 领域分包与 B6-3、v1.7.3 配置目录 / 事件总线 / B6-4、v1.7.4 更新完整性 / Webhook 边界 / 诊断与快速开始、v1.8.0 SEGGER J-Link RTT。
...
| 版本策略 | 阶段 A/B → **v1.5.3 已发**；打磨向 → **v1.5.4–v1.5.7**；**v1.6.0 已发**（协议流组帧）；**v1.7.0 已发**（Windows BLE 主机 UART）；**v1.7.1 已发**（扫描打磨）；**v1.7.2 已发**（领域分包 / B6-3）；**v1.7.3 已发**（配置目录 / 事件总线 / B6-4）；**v1.7.4 已发**（更新完整性 / Webhook 边界 / 诊断与快速开始）；**v1.8.0**（SEGGER J-Link RTT） |
...
| 指标 | 数值 | 备注 |
|---|---|---|
| 发布版 | `comm-v1.8.0` | Windows Setup/onefile；macOS DMG 与 Linux x86_64 `.run` 完成后追加同一 tag；清单须带各平台 `sha256` / `size` |
| 测试 | **1784 collected** | Windows 全量 pytest、RTT 专项、Ruff 与编译检查已通过 |
...
12. **不上**覆盖率硬门槛（B3 只收集，见 §6）
13. 阶段 C 剩余项（Mac 公证）与 P2 / PyQt6 仅按触发条件启动；Linux CI 烟雾已加
...
- **v1.8.0 之后「接下来做什么」** → 以本文件 §5 / §7 为准（阶段 C 按触发条件；不上 P2）。
