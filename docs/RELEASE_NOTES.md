...
- **本轮先发 Windows + Linux x86_64**；macOS `.dmg` 仍补同一 tag `comm-v1.6.0`（校验后再写入 `url_mac`）
...
| Linux（x86_64） | `CommTool_Setup_v1.6.0_linux_x86_64.run` | 免 sudo，默认 `~/.local/opt/CommTool`；glibc ≥ 2.27（Ubuntu 18.04+ / 多数麒麟） |
