...
> v1.5 多会话：独占引擎**按会话独立**（脚本/MBM/录制/宏/DSL/扫描/序列；软切标签；关闭忙标签前需先停任务）；后台 RX 喂本会话引擎、自动应答与触发器；仍不做会话树/分屏/拖出。
...
- 序列 / 脚本 / Modbus 主机 / 录制 / 宏 / 发送 DSL：运行态 per-session（双标签可各跑）；传输 / 回放仍同窗一份并钉会话
- 设备扫描：仍同窗一份（临时接管窗口 `_mbm_rules`）；占用记在启动标签
- 自动应答：窗口级开关 + 每会话运行缓冲与 **从机寄存器 bank**；后台 RX 亦喂 AR
- 触发器：按会话隔离解码流，后台 RX/TX 也喂；序列对话框汇总其它标签运行态，Run/Stop 仍只作用于当前标签
- 非目标：会话树、拖拽分屏、标签拖出成窗
